...
public class InitRedisCache {
    ...
    public void initCache() {
        ...
        //缓存热门赛事和屏蔽
        List<SportHotEntity>sportHots=sportsHotMapper.queryAll();
        List<SportsShieldEntity> sportsShields=sportsShieldMapper.queryAll();
        sportHots.forEach(p -> redisUtil.sSet(Constant.SPORT_HOT_SALT,p.getMid()));
        for(SportsShieldEntity shieldEntity:sportsShields){
            if(1==shieldEntity.getType()){
                redisUtil.sSet(Constant.SPORT_SHIELD_SALT,shieldEntity.getMId());
            }else{
                redisUtil.sSet(Constant.SPORT_MLEAGUE_SHIELD_SALT,shieldEntity.getMId());
            }
        }
